# 処理キュー（50件達成まで。達成後に削除）

書式: URL | 説明 | TTM売上/利益 | 価格 | 状態

## 処理予定（優先度順）

- /startup/GMlFhUQPHsfiaItjdswKpvxvDEC2/ziMQaZModGayB3qCCN8U | AI見積SaaS（建設業者向け） | $159k/$135k | $675k | done #011 見送り
- /startup/H8lhr66seCQChOq4YbgjFN0EBTi2/Kb2Qpk6EWExkBYodMLO5 | 小売店向け顧客エンゲージメント | $1.2M/$211k | $1.3M | done #012 見送り
- /startup/crFgXi79noYVLzpzbsFUFioxaUi1/ygFePVsvf2tX16EYV5Ac | AIカスタマーサービス | $484k/$40k | $1.3M | done #013 スキップ
- /startup/zRaHq4jQ4HXBfGiHAB3uUH75gUN2/rAnCcYdPoBRxm2OxxqiG | 集団訴訟和解金アプリ | $1.1M/$805k | $2.8M | done #014 スキップ
- /startup/rPp24VTssgVRoPN9OAc1PZPVtz72/y3dxJpeDv021nU7ONJ7J | Slack営業会話分析 | $40k/$27k | $137k | done #015 スキップ
- /startup/UqAncAy2V4hvgU33lIOvPnpH1jf1/LaPSuQt4RPSP2is9FXNu | AI画像生成PF（GPU込み） | $160k/$76k | $365k | done #016 スキップ
- /startup/R9MLgv52MZZn4NRfsSHe0DCfY7M2/RfwIrrhRPOtTn6owKgis | 女性向けメディアブランド | $249k/$126k | $378k | done #017 スキップ
- /startup/FWbFkG2po7evMkLh2yKDlGpIooI3/SePy0fpR1lilQiecPd3W | 自己啓発学習アプリ | $1M/$250k | $5M | done #018 スキップ
- /startup/WlzEqMIy5gXTaGrjbS36gdKNDT93/AVFuydD7yRqoZudYVsVf | UK GLP-1オンライン薬局 | $3M/$182k | $3M | done #019 スキップ（ドメイン）
- /startup/xkdFhTiYO3V0JhgvqYAgwr9TkxE3/oFNSatezTKMgEgn9TXCA | サプリEC | $6.1M/$1.1M | $5.7M | done #020 スキップ

## カード段階でスキップ（詳細を開かない。理由つき）

- 顔分析AI $3k / 面接練習 $3k / 会話コーチ $5k / QRコード $0 / フィットネス白label $0 / AI動画営業 $599 / 3D写真 $7k / デートアプリ $8k / アスリート税務 $0 — 売上ほぼゼロ、客単価・チャーンのデータが取れない

## 未収穫（次回の収穫で URL を拾う）

- Lovable競合 $284k/$151k/$600k、YouTubeツール $242k/$195k/$800k、GEO/AEOマーケ $124k/$79k/$250k、サイト→アプリ変換 $1.2M/$761k/$4M、トレーダー向け $580k/$258k/$600k、VoIP $354k/$79k/$450k、SOP管理 $87k/$57k/$249k、白label代理店 $676k/$112k/$336k、AIコードレビュー $406k/$128k/$550k、AIアポ設定 $128k/$109k/$189k、教会説教AI $275k/$120k/$500k、K-12教師AI $227k/$192k/$200k、インテントデータ $373k/$319k/$1.5M、イベント駐車場転売 $1.2M/$377k/$700k、イベント写真共有 $35k/$31k/$153k、在宅サービスリード $30k/$13k/$100k

## 追加収穫（イテレーション3）

- /startup/3M9q48z12LbFKwhFk4JuI7fS84r1/Ef91596cOS3xop739GSx | YouTubeツールSaaS | $242k/$195k | $800k | done #021 スキップ
- /startup/jfPbtpdxplOFzNvPGsFPvtt7IE12/XKVQVT4aaKXAvDDfesb6 | Shopifyテーマ事業 | $1.5M/$1.4M | $2.7M | done #022 見送り
- /startup/BLpkQCNu2ZhI42YlmUrLh1ZmJsS2/nvqesxbd3P7LWlSADofq | アフリカ農家Ag-tech/Fintech | $9.1M/$2.9M | $12.8M | pending（規模外だがデータ行候補・低優先）
- /startup/GHYkTTqJ5Ce5l9524nitCtE8LqE2/8FLDQTHD8HxUSyi95vzp | トレーディングアルゴ | $58k/$57k | $190k | pending（投資助言ドメイン・低優先）
- カード段階スキップ追加: 田舎求人PF $25k/$3k、テスト済みEC店 $200k/$8k（利益0.24x表記で不審）

## 追加収穫（イテレーション4）

- /startup/iAQ7KHQSumchENwzNjlflWT4I5p2/3vYgvw99KnWjbD1IigDl | No-Code AIエージェント | $64k/$55k | $255k | done #023 スキップ
- /startup/knTQCyk7E0PrC0UGJsQ1IXkwTEa2/t8uPrYDRTcJOWuOEpfaO | トレーダー向けPF | $148k/$120k | $425k | done #024 スキップ
- /startup/ZiOJlEbU2eSA4c31Mi9S468Oh5h1/5pLxqOcUW3dLFF35csbo | データ分析コンサル | $90k/$85k | $70k | done #025 スキップ
- /startup/IafR67ENRoYBICFyOxWtUetuXOy2/zml0Q62UZFUUiFHVyK2h | AI CS小型 | $89k/-$13k | $90k | done #026 スキップ
- カード段階スキップ追加: スポーツ栄養ドロップシップ $52k/$0（利益ゼロEC）、Newsletter-to-eReader $620
